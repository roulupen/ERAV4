...
class Trainer:
    ...
    def __init__(self, model, device, train_loader, test_loader, 
                 optimizer, criterion, scheduler=None, config=None, l2_lambda=0.0):
        """
        ...
        Args:
            ...
            l2_lambda: L2 regularization strength (optional, default: 0.0)
        """
        self.model = model
        self.device = device
        ...
        # Print L2 regularization info
        if self.l2_lambda > 0:
            print(f"🔧 L2 regularization enabled with lambda = {self.l2_lambda}")
        else:
            print(f"🔧 L2 regularization disabled (lambda = {self.l2_lambda})")
    
    def calculate_l2_regularization(self) -> float:
        """
        Calculate L2 regularization term for all model parameters.
...
def create_trainer(model, device, train_loader, test_loader, optimizer, criterion, 
                  scheduler=None, config=None, l2_lambda=0.0) -> Trainer:
    """
    ...
    """
    return Trainer(model, device, train_loader, test_loader, optimizer, criterion, scheduler, config, l2_lambda)
...
def train_model(model, device, train_loader, test_loader, optimizer, criterion, scheduler=None,
                ...
                checkpoint_dir='./checkpoints', scheduler_type='step', config=None, l2_lambda=0.0) -> Dict[str, Any]:
    """
    ...
    trainer = create_trainer(model, device, train_loader, test_loader, 
                           optimizer, criterion, scheduler, config, l2_lambda)
